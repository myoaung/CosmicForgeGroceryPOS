...
class MockSyncService extends SyncService {
  MockSyncService(LocalDatabase db) : super(db, null, null);
  @override
...
void main() {
  ...
  setUp(() async {
    ...
    cartNotifier =
        CartNotifier(cartRepo, checkout, 5.0, MockSyncService(db)); // 5% tax
  });
  ...
  test('addToCart adds item and updates totals', () async {
    ...
    await cartNotifier.addToCart(product, tenantId: 'tn1', storeId: 's1');
  ...
  test('Tax Exemption Logic', () async {
    final rice = await (db.select(db.products)..where((t) => t.id.equals('p1')))
        .getSingle();

    await cartNotifier.addToCart(rice, tenantId: 'tn1', storeId: 's1');
